...
#include  "hello_world.h"  // Various enums and struct declarations
...
void  cbDraw (Canvas* const canvas,  void* ctx)
{
	...
	switch (state->animID) {
		...
		case ANIM_COFFEE:
			canvas_set_font(canvas, state->fontTwo);
			canvas_draw_str_aligned(canvas, state->x, state->y, AlignLeft, AlignTop, state->textTwo);
			break;
...
bool  stateInit (state_t* const state,  const Gui* gui)
{
	...
	state->fontTwo    = FontKeyboard;
	if (!(state->text = strdup("OGText"))) {
		ERROR(errs[ERR_MALLOC_TEXT]);
		rv = false;
		goto bail;
	}

	if (!(state->textTwo = strdup("NewText"))) {
		ERROR(errs[ERR_MALLOC_TEXT]);
		rv = false;
		goto bail;
	}
...
// Oh no! A horrible scary "goto" label!B
