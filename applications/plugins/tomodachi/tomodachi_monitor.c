...
static void tomodachi_app_draw_callback(Canvas* canvas, void* ctx) {
    ...
    char ttl_str[10] = {0};
    snprintf(ttl_str,sizeof(ttl_str), "TTL: %d", tomodachi->last_message[TTL_POS]);
    ...
    snprintf(timestamp_str, sizeof(timestamp_str), "Timestamp: %ld", (uint32_t)timestamp);
    canvas_draw_str(canvas, 0, 30, timestamp_str);
    free(timestamp);
    // Print known devices count
    char known_devices_str[20] = {0};
    snprintf(known_devices_str, sizeof(known_devices_str), "Known devices: %d", tomodachi->tomodachi_known_count);
