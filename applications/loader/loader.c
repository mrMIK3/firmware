...
const FlipperApplication* loader_find_application_by_name(const char* name) {
    ...
    if(!application) {
        application = loader_find_application_by_name_in_list(name, FLIPPER_GAMES_APPS, FLIPPER_GAMES_APPS_COUNT);
    }	
    if(!application) {
...
void loader_cli_list(Cli* cli, string_t args, Loader* instance) {
    ...
    printf("Games:\r\n");
    for(size_t i = 0; i < FLIPPER_GAMES_APPS_COUNT; i++) {
        printf("\t%s\r\n", FLIPPER_GAMES_APPS[i].name);
    }
...
static void loader_build_submenu() {
    ...
    for(i = 0; i < FLIPPER_GAMES_APPS_COUNT; i++) {
        submenu_add_item(
            loader_instance->games_menu,
            FLIPPER_GAMES_APPS[i].name,
            i,
            loader_menu_callback,
            (void*)&FLIPPER_GAMES_APPS[i]);
    }
